// Seeds Research data into the database from the structured content currently
// hard-coded in app/(pages)/research/page.tsx. Per Steps.md 5-1, the old
// Gnuboard HTML blob is intentionally not the source — the page already holds
// the cleaned-up version that matches the live site's four topics / seventeen
// subsections. Run with: npm run db:seed-research

import { prisma } from "../lib/db";

// ─── Hero / meta ─────────────────────────────────────────────────────────────

const HERO_HEADLINE = "Engineering heat\nat every scale.";
...
const YEARS_VALUE = "10+";

// ─── Topics / subsections / figures ──────────────────────────────────────────

type FigureSeed = { w: number; h: number; caption: string };
type SubSeed = {
  num: string;
  title: string;
  body: string;
...
const RESEARCH_TOPICS: TopicSeed[] = [
  {
    num: "01",
    bg: "white",
...
async function main() {
  console.log("Wiping Research tables…");
  // Figure / Subsection rows cascade on Topic delete, so dropping topics is enough.
  await prisma.researchTopic.deleteMany();
  await prisma.researchPageMeta.deleteMany();

  console.log("Seeding ResearchPageMeta…");
  await prisma.researchPageMeta.create({
    data: {
      heroHeadline: HERO_HEADLINE,
      heroParagraph: HERO_PARAGRAPH,
      yearsValue: YEARS_VALUE,
    },
  });

  console.log("Seeding Topics / Subsections / Figures…");
  for (let ti = 0; ti < RESEARCH_TOPICS.length; ti++) {
    const t = RESEARCH_TOPICS[ti];
  ...
  const counts = {
    topics: await prisma.researchTopic.count(),
    subsections: await prisma.researchSubsection.count(),
    figures: await prisma.researchFigure.count(),
    meta: await prisma.researchPageMeta.count(),
  };
