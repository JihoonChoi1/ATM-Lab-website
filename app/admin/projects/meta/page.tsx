...
export const metadata: Metadata = { title: "Projects page meta · ATM Lab" };

// Reads the session cookie + the singleton row → never cache.
export const dynamic = "force-dynamic";

export default async function ProjectsMetaPage({
  searchParams,
}: {
  searchParams: { saved?: string };
}) {
  await requireAdmin("/admin/projects/meta");

  const meta = await prisma.projectsPageMeta.findFirst();
  const defaults = PAGE_HERO_DEFAULTS.projects;

  return (
    <div className="mx-auto w-full max-w-[640px]">
      <div className="mb-8">
        <h1 className="text-3xl font-bold tracking-[-0.02em]">Projects page meta</h1>
        <p className="mt-1 text-sm text-ink-3">
          The hero heading and intro paragraph at the top of the public /projects page.
        </p>
      </div>
